...
def get_pump_recommendation(answers_dict):
    ...
    if depth is not None:
        ...
        if depth <= 900 and level == 'high' and sand == 'high':
            add("RHT", +70, "Для неглубоких скважин с высоким уровнем нефти и повышенным содержанием песка. — RHT рекомендовано")
...
def get_cylinder_recommendation(answers_dict, flow_rec=None, dlina_plunger=None):
    ...
    # Верхнее крепление (A) — базовые значения из таблицы
    "20-106 RHAM":          1.211,
    "20-125 RHAM":          1.214,

    # Верхнее крепление (ОСТ) — тот же узел крепления, тот же К
    "20-106 RHAM (OCT)":    1.211,
    "20-125 RHAM (OCT)":    1.214,

    # Верхнее крепление, манжетное — узел крепления тот же, К идентичен RHAM
    "20-106 RHAC":          1.211,
    "20-125 RHAC":          1.214,

    # Нижнее крепление — базовые значения из таблицы
    "20-106 RHBM":          1.178,
    "20-125 RHBM":          1.178,

    # Нижнее крепление, манжетное — узел тот же, К = RHBM
    "20-106 RHBC":          1.178,
    "20-125 RHBC":          1.178,

    # 20-150 RHBC — нет в таблице, берём как 20-125 RHBM (ближайший нижний, 60.3)
    "20-150 RHBC":          1.178,

    # RHM-T (узел перенесён вниз) — по конструкции близок к RHBM
    "20-106 RHM-T":         1.178,
    "20-125 RHM-T":         1.178,
    "20-175 RHM-T":         1.178,
    "20-175 RHM-TП":        1.178,
    ...
    # ══════════════════════════════════════════════════════════════
    # RH ВСТАВНЫЕ — НКТ 73,0 мм (размеры 106, 125 с увел. узлом; 150, 175)
    # ══════════════════════════════════════════════════════════════

    # 25-106 / 25-125 с увеличенным узлом — нет в таблице,
    # К берём от 25-150 RHAM / RHBM как ближайших из той же НКТ 73,0
    "25-106 RHAM-T":        1.266,   # как 25-150 RHAM
    "25-125 RHAM-T":        1.266,   # как 25-150 RHAM
    "25-106 RHAM (Конус)":  1.266,
    "25-125 RHAM (Конус)":  1.266,
    "25-106 RHBM":          1.289,   # как 25-150 RHBM
    "25-125 RHBM":          1.289,
    "25-106 RHAC":          1.266,
    "25-125 RHAC":          1.266,
    "25-106 RHBC":          1.289,
    ...
    # 25-150 — базовые значения из таблицы
    "25-150 RHAM":          1.266,
    "25-150 RHBM":          1.289,

    # 25-150 прочие исполнения — К по типу крепления
    "25-150 RHAM (OCT)":    1.266,
    "25-150 RHAM (Конус)":  1.266,
    "25-150 RHAC":          1.266,
    "25-150 RHBC":          1.289,
    "25-150 RHM-T":         1.289,

    # 25-175 — базовые значения из таблицы
    "25-175 RHAM":          1.352,
    "25-175 RHBM":          1.375,

    # 25-175 прочие исполнения
    "25-175 RHAM-П":        1.352,
    "25-175 RHAM (OCT)":    1.352,
    "25-175 RHAM (Конус)":  1.352,
    "25-175 RHAC":          1.352,
    "25-175 RHBC":          1.375,
    "25-175 RHM-T":         1.375,
    "25-175 RHM-TП":        1.375,

    # 25-175 RHTM (ОСТ) — подвижный цилиндр, нижнее крепление → как RHBM
    "25-175 RHTM (OCT)":    1.375,

    # ══════════════════════════════════════════════════════════════
    # RH ВСТАВНЫЕ — НКТ 88,9 мм (размер 225)
    # ══════════════════════════════════════════════════════════════

    # 30-225 — нет в исходной таблице; ближайший по конструкции — 25-175 RHBM (нижнее)
    "30-225 RHBM":          1.375,
    "30-225 RHBM (2-ст)":   1.375,
    "30-150 RHBM (2-ст)":   1.375,

    # ══════════════════════════════════════════════════════════════
    # TH ТРУБНЫЕ — НКТ 48,3 мм
    # ══════════════════════════════════════════════════════════════

    # 15-125 THM — нет в таблице; ближайший меньший — 20-125 THM
    "15-125 THM":           1.614,

    # ══════════════════════════════════════════════════════════════
    # TH ТРУБНЫЕ — НКТ 60,3 мм
    # ══════════════════════════════════════════════════════════════

    "20-125 THM":           1.614,   # из таблицы

    "20-175 THM":           1.877,   # из таблицы
    "20-175 THM-A":         1.877,

    # ══════════════════════════════════════════════════════════════
    # TH ТРУБНЫЕ — НКТ 73,0 мм
    # ══════════════════════════════════════════════════════════════

    # 25-125 THM — нет в таблице; ближайший — 20-125 THM (тот же диаметр плунжера)
    "25-125 THM":           1.614,
    "25-125 THM-T":         1.614,
    "25-125 THM-C":         1.614,
    "25-125 THM-СЛ":        1.614,

    # 25-106 THM — нет в таблице; диаметр меньше 125, берём 20-125 THM
    "25-106 THM-C":         1.614,
    "25-106 THM-СЛ":        1.614,

    # 25-150 THM — нет в таблице; между 20-125 (1.614) и 20-175 (1.877),
    # интерполируем или берём ближайший больший → 20-175 THM
    "25-150 THM-C":         1.877,
    "25-150 THM-СЛ":        1.877,

    # 25-175 THM — из таблицы (как 20-175 THM, нет отдельного значения)
    "25-175 THM":           1.877,
    "25-175 THM-A":         1.877,
    "25-175 THM-T":         1.877,
    "25-175 THM-TA":        1.877,
    ...
    "25-175 THM-CA":        1.877,
    "25-175 THM-СП":        1.877,
    "25-175 THM-СПА":       1.877,
    "25-175 THM-СЛ":        1.877,
    "25-175 THM-СЛА":       1.877,
    "25-175 THC":           1.877,
    "25-175 THC-A":         1.877,

    # 25-225 THM — из таблицы
    "25-225 THM":           2.080,
    "25-225 THM-T":         2.080,
    "25-225 THM-C":         2.080,
    "25-225 THM-CA":        2.080,
    "25-225 THM-CП":        2.080,
    "25-225 THM-CПА":       2.080,
    "25-225 THM-СЛ":        2.080,

    # ══════════════════════════════════════════════════════════════
    # TH ТРУБНЫЕ — НКТ 88,9 мм
    # ══════════════════════════════════════════════════════════════

    "30-275 THM":           2.293,   # из таблицы
    "30-275 THM-T":         2.293,
    "30-275 THM-C":         2.293,
}
    ...
    # ══════════════════════════════════════════════════════════════
    # RH ВСТАВНЫЕ — НКТ 60,3 мм
    # ══════════════════════════════════════════════════════════════

    # 20-106 RHAM / 20-125 RHAM (крепл. API) — стр. 40, строка 1
    "20-106 RHAM": {
    ...
    # 20-106 RHAM (ОСТ) / 20-125 RHAM (ОСТ) — стр. 40, строка 2
    "20-106 RHAM (OCT)": {
    ...
    # 20-106 RHBM / 20-125 RHBM — стр. 40, последняя строка
    "20-106 RHBM": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5, 6],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 20-106 RHAC / 20-106 RHBC / 20-125 RHAC / 20-125 RHBC — стр. 41, строка 3
    "20-106 RHAC": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 20-150 RHBC — стр. 42, отдельная строка (только 14', без удлинителей)
    "20-150 RHBC": {
        "cylinders": [14],
        "plungers":  [4],
        "ext_pairs": [],  # «Отсутствуют»
    },

    # ══════════════════════════════════════════════════════════════
    # RH ВСТАВНЫЕ — НКТ 73,0 мм (увеличенный узел крепления, 25-xxx)
    # ══════════════════════════════════════════════════════════════

    # 25-106 RHAM-T / 25-125 RHAM-T (крепл. API, увел. узел) — стр. 40, строка 4
    "25-106 RHAM-T": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25/20-106 RHM-T / 25/20-125 RHM-T (узел перенесён вниз) — стр. 40, строка 5
    "20-106 RHM-T": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25-106 RHAM (Конус) / 25-125 RHAM (Конус) — стр. 40, строка 3
    "25-106 RHAM (Конус)": {
        "cylinders": [7, 8, 9, 10, 12, 14, 15],
        "plungers":  [4, 5, 6],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    },
    "25-125 RHAM (Конус)": {
    ...
    # 25-106 RHBM / 25-125 RHBM (увел. узел) — стр. 41, строка 1
    "25-106 RHBM": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25-106 RHAC / 25-106 RHBC / 25-125 RHAC / 25-125 RHBC (увел. узел) — стр. 41, строка 4
    "25-106 RHAC": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25-150 RHAM / 25-175 RHAM (крепл. API) — стр. 41, строка 5
    # + 25-175 RHAM-П
    "25-150 RHAM": {
    ...
    # 25-150 RHAM (ОСТ) / 25-175 RHAM (ОСТ) — стр. 41, строка 6
    "25-150 RHAM (OCT)": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5, 6],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25-150 RHAM (Конус) / 25-175 RHAM (Конус) — стр. 41, строка 7
    "25-150 RHAM (Конус)": {
        "cylinders": [7, 8, 9, 10, 12, 14],
        "plungers":  [4, 5, 6],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25/20-150 RHM-T / 25/20-175 RHM-T / 25/20-175 RHM-ТП — стр. 41, строка 8
    "25-150 RHM-T": {
        "cylinders": [7, 8, 9, 10, 12, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    },
    "25-175 RHM-T": {
    ...
    # 25-150 RHBM / 25-175 RHBM — стр. 41, последняя строка + стр. 42
    "25-150 RHBM": {
    ...
    # 25-150 RHAC / 25-150 RHBC / 25-175 RHAC / 25-175 RHBC — стр. 42
    "25-150 RHAC": {
        "cylinders": [7, 8, 9, 10, 12, 14, 16],
        "plungers":  [4, 5],
        "ext_pairs": [(3, (1.5, 1.5)), (4, (2.0, 2.0))],
    ...
    # 25-175 RHTM (ОСТ) — стр. 42 (только 12', плунжер 4', удл. 4'(2x2'))
    "25-175 RHTM (OCT)": {
        "cylinders": [12],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 30-225/150 RHBM (двухступенчатый) — стр. 42
    "30-225 RHBM (2-ст)": {
        "cylinders": [12],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 20-175 THM / 20-175 THM-A — стр. 43
    "20-175 THM": {
    ...
    # ══════════════════════════════════════════════════════════════
    # TH ТРУБНЫЕ — НКТ 73,0 мм (увел. верхняя муфта, 25-xxx)
    # ══════════════════════════════════════════════════════════════

    # 25-125 THM (увел. муфта) — стр. 42
    "25-125 THM": {
        "cylinders": [7, 8, 9, 11, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(4, (2.0, 2.0)), (5, (2.0, 3.0))],
    },

    # 25-125 THM-T — стр. 42 (11', 14'; плунжер 4'; удл. 4'(2x2'))
    "25-125 THM-T": {
        "cylinders": [11, 14],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 25-106 THM-C / 25-125 THM-C / 25-150 THM-C — стр. 42
    # (11', 14', 22'(2x11'); плунжер 4', 5'; удл. 2'(2x1'), 3'(2'+1'), 4'(2x2'))
    "25-106 THM-C": {
        "cylinders": [11, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(2, (1.0, 1.0)), (3, (2.0, 1.0)), (4, (2.0, 2.0))],
    },
    "25-125 THM-C": {
        "cylinders": [11, 14, 22],
        "plungers":  [4, 5],
    ...
    # 25-106 THM-СЛ / 25-125 THM-СЛ / 25-150 THM-СЛ — стр. 42 (те же параметры)
    "25-106 THM-СЛ": {
        "cylinders": [11, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(2, (1.0, 1.0)), (3, (2.0, 1.0)), (4, (2.0, 2.0))],
    ...
    # 25-175 THM / 25-175 THM-A — стр. 43
    "25-175 THM": {
    ...
    # 25-175 THM-T / 25-175 THM-TA — стр. 43 (11', 14'; плунжер 4'; удл. 4'(2x2'))
    "25-175 THM-T": {
        "cylinders": [11, 14],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 25-175 THM-C / THM-CA / THM-СП / THM-СПА / THM-СЛ / THM-СЛА — стр. 43
    # (9', 11', 14', 22'(2x11'); плунжер 4', 5'; удл. 2'(2x1'), 3'(2'+1'), 4'(2x2'))
    "25-175 THM-C": {
        "cylinders": [9, 11, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(2, (1.0, 1.0)), (3, (2.0, 1.0)), (4, (2.0, 2.0))],
    },
    "25-175 THM-CA": {
    ...
    # 25-175 THC / 25-175 THC-A — стр. 43 (только 11'; плунжер 4'; удл. 5'(2'+3'))
    "25-175 THC": {
        "cylinders": [11],
        "plungers":  [4],
        "ext_pairs": [(5, (2.0, 3.0))],
    ...
    # 25-225 THM-T — стр. 43 (11', 14'; плунжер 4'; удл. 4'(2x2'))
    "25-225 THM-T": {
        "cylinders": [11, 14],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 25-225 THM-C / THM-CA / THM-СП / THM-СПА / THM-СЛ — стр. 43-44
    "25-225 THM-C": {
        "cylinders": [11, 14, 22],
        "plungers":  [4, 5],
        "ext_pairs": [(2, (1.0, 1.0)), (3, (2.0, 1.0)), (4, (2.0, 2.0))],
    ...
    # 30-275 THM-T — стр. 44 (11', 14'; плунжер 4'; удл. 4'(2x2'))
    "30-275 THM-T": {
        "cylinders": [11, 14],
        "plungers":  [4],
        "ext_pairs": [(4, (2.0, 2.0))],
    ...
    # 30-275 THM-C — стр. 44 (11', 14'; плунжер 4'; удл. 2'(2x1'), 3'(2'+1'), 4'(2x2'))
    "30-275 THM-C": {
        "cylinders": [11, 14],
        "plungers":  [4],
        "ext_pairs": [(2, (1.0, 1.0)), (3, (2.0, 1.0)), (4, (2.0, 2.0))],
